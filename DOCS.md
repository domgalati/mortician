...
- [Shell Completion (WSL/Linux)](#shell-completion-wsllinux)
- [Tips](#tips)
...
```text
mortician {create,select,edit,add,action,show,list,timeline,serve} ...
```
...
- `--render` or `--render rich`: Rich-based rendering in the terminal.
...
```text
mortician action add|list|done|undo ...
```

Manages `actions.yaml` for the active incident:

- `add` — new item (`--task` or stdin; optional `--owner`, `--due`)
- `list` — checkbox-style listing (shows 1-based indices)
...
- follow-up actions (`mortician action`) via `actions.yaml`
